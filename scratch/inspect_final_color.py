...
async def main():
    async with async_playwright() as p:
        ...
        if await toggle_label.count() > 0:
            ...
            # Print styles of the track
            track_style = await toggle_label.locator("> *:first-child").evaluate("""el => {
            ...
            # Print text background color to verify it's transparent
            text_el = toggle_label.locator("div[data-testid='stWidgetLabel']")
            if await text_el.count() > 0:
                text_style = await text_el.first.evaluate("""el => {
                    const style = window.getComputedStyle(el);
                    return {
                        backgroundColor: style.backgroundColor,
                        color: style.color
                ...
                safe_print(f"Text container style: {text_style}")
